...
class ProdutoController(
    ...
    fun listarSimples() : ResponseEntity<List<ProdutoSimplesResponse>> {
        val dtos = produtoService.getLista()
...
//    @PostMapping
    ...
    @PostMapping
    fun criar(@RequestBody @Valid novoProduto: Produto) : ResponseEntity<ProdutoResponse> {
        val produto = produtoService.salvar(novoProduto)
        return ResponseEntity.status(201).body(produto)
    }

    @PutMapping("/{id}")
    fun atualizar(@PathVariable id:Int, @RequestBody @Valid produto: Produto) : ResponseEntity<ProdutoResponse> {
        produto.id = id
        val produto = produtoService.salvar(produto)
        return ResponseEntity.status(200).body(produto)
    }

    @GetMapping
    fun listar() : ResponseEntity<List<ProdutoResponse>> {
        val produtos = produtoService.getListaCompleta()
        return ResponseEntity.status(200).body(produtos)
    }

    @GetMapping("/{id}")
    fun listarPeloId(@PathVariable id:Int) : ResponseEntity<ProdutoResponse> {
        val produtos = produtoService.get(id)
        return ResponseEntity.status(200).body(produtos)
    }

    @GetMapping("/financeiro")
    fun listarFinanceiro() : ResponseEntity<List<ProdutoFinanceiroResponse>> {
        val produtos = produtoService.getListaFinanceiroCompleta()
        return ResponseEntity.status(200).body(produtos)
    }
